...
Имя: tableManager
Версия: 1.1.0
Автор: Пьетрантонио Алессандро (Камень)
Сайт автора: http://www.stonewebdesign.it

@-- Какие новости --@

1)
    Новые классы в таблице: tablePagination, tableFilterBy;

2)
    Новый класс для элемента th: disableFilterBy;

3)
    Новая опция отключения фильтра для одного или нескольких столбцов: disableFilterBy: []

@-- Применение --@

Важный! Для работы этого плагина НЕОБХОДИМА библиотека jQuery. По сути, это плагин jQuery.

Минимальные требования:
- библиотека jQuery

1)
    Чтобы включить библиотеку jQuery, загрузите ее с сайта jQuery и поместите файл .js в папки вашего сайта, а затем поместите эту строку в свой документ:
    <script type = "text / javascript" src = "PATH_TO_YOUR_FILE / jquery-VERSION_NUMBER.min.js"> </script>
    В качестве альтернативы вы можете включить его без загрузки, вот так:
    <script src = "https://ajax.googleapis.com/ajax/libs/jquery/1.12.2/jquery.min.js"> </script>
2)
    Включите этот плагин следующим образом: загрузите его и поместите файл .js в папки вашего сайта, затем напишите эту строку в свой документ:
    <script type = "text / javascript" src = "PATH_TO_YOUR_FILE / tableManager.js"> </script>
    Важный! ПОМНИТЕ: ЭТА СТРОКА ДОЛЖНА БЫТЬ ПОСЛЕ JQUERY, ВКЛЮЧАЯ СТРОКУ!


Если вы хотите, вы можете настроить этот плагин по своему усмотрению и потребностям, используя следующие параметры.
Параметры:

    debug = (boolean) может иметь значение true, false или не задано. Он активирует режим отладки и отображает сообщения в консоли браузера.

    firstSort = может быть массивом целых чисел или просто одним целым числом. Первый параметр определяет номер столбца для начала сортировки, второй параметр определяет порядок (asc или 0 = по возрастанию, desc или 1 = по убыванию). Пример:
        firstSort: [[1, 'asc']] -> таблица сортируется по первому столбцу (первый параметр = 1), по возрастанию (второй параметр = 0).

    disable = этот параметр используется для отключения одного или нескольких столбцов и ожидания одного параметра для каждого столбца. Параметр может быть целым числом или, чтобы отключить последний столбец, -1 или словом «последний». Пример:
        disable: [3] -> отключить сортировку в третьем столбце

    appendFilterby = (boolean) используется для добавления фильтра поверх таблицы. Фильтр будет состоять из одного выбора, по которому нужно выбрать столбец для фильтрации, и введенного текста для фильтрации ввода. Например:
        appendFilterby: true

    dateFormat = используется для обозначения столбца и формата даты. Это помогает сортировать по дате в формате дд-мм-гггг или ммддйгг. Первый параметр - это столбец, второй - формат даты. Пример:
        dateFormat: [[3, 'dd-mm-yyyy']] -> третий столбец - это дата и отформатирован как dd-mm-yyyy

    pagination = true или false или не установлено. Это позволяет разбивать таблицу на страницы и добавлять контроллеры под таблицей. Например:
        pagination: true -> включить инструмент разбивки на страницы

    showrows = вы можете добавить к таблице выбор, с помощью которого вы можете выбрать количество отображаемых строк. Это должен быть массив чисел. Пример:
        showrows: [10,100,1000] -> вы можете выбрать отображение 10, 100, 1000 строк

    словарь = используется для перевода этикеток. Допускаются следующие этикетки:
        voice_filter_by
        voice_type_here_filter

    disableFilterBy = используется для отключения фильтра по определенным столбцам. Это должен быть массив чисел. Чтобы отключить фильтр по последнему столбцу, вы можете использовать «последний». Например:
        disableFilterBy: [1, "last"] -> отключить фильтр по первому и последнему столбцу

Классы:

    (th) disableSort = отключить этот конкретный столбец
                disableFilterBy = отключить фильтр для этого конкретного столбца

    (table) tablePagination = добавить элементы нумерации страниц в таблицу
                tableFilterBy = добавить инструмент фильтра в таблицу

Атрибуты данных:

    data-tablemanager =
        'disable' -> отключить этот конкретный столбец

        '{"dateFormat": "dd-mm-yyyy"}' -> этот отдельный столбец представляет дату в формате tdd-mm-yyyy (Важно! Чтобы правильно передать этот атрибут данных, значение атрибута должно быть записано между ' 'и указать отдельные элементы, такие как dateFormat или mm-dd-yyyy между "")


Важный! Не редактируйте этот плагин, если не уверены, что делаете это правильно. Автор не несет ответственности за какие-либо неисправности, вызванные конечным пользователем.
...
(function ($) {
    ...
    $.fn.tablemanager = function (options = null) {
        ...
        **/
        ...
        var currentPage = 0;
        var numPerPage =
            pagination !== true && showrows_option !== true ? rows.length : 2;
        ...
        **/
        function TableSort(rules) {
            ...
            // For each firtsort rule -- для каждого правила первой сортировки
            var firstSortData = [];
            for (i = 0; i < rules.length; i++) {
                ...
                var order =
                    rules[i][1] == undefined
                        ...
                        ? 1
                        : 0;
                // Manage classes asc and desc--Управление классами по возрастанию и по убыванию
                if (i == 0) {
                    var firstClassOrder =
                ...
                asc = order == 0 ? 1 : -1;
                // if column is date -- если столбец дата
