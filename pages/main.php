<?php 
  ...
  //выводим информацию о картинках по категориям из Б/Д 
  $id_cat = category_id($categories);
  //Запрос - номер Categories
  $request = read_request();
  //Проверка на сушествования фотографий в папке
  $array_images = array_images(null,$request,null);
?>

<!--Плагин wookmark - расположение картинок в колонны-->
<script src="script/wookmark/wookmark.js"></script>

<!-- Once the page is loaded, initalize the plug-in / После загрузки страницы инициализируйте плагин -->
<script>
    window.onload = function () {
      var wookmark = new Wookmark('#wookmark', {
        outerOffset: 5, // Optional, the distance to the containers border + Необязательно, расстояние до границы контейнера
        itemWidth: 210 // Optional, the width of a grid item + Необязательно, ширина элемента сетки
      });
    };
</script>
<!--Условия на пустоту папок с фотографиями-->
<?php
if (isset($array_images[0]) || $request == 'all' || $page == null || $categories == null){ // если папка не пуста/выведен раздел - all/$categories/$page - в строке не указаны?>
  <!--Выводим фотографии-->
    <?php
      if ($categories == 'all' || $request == null ) {//Все КАТЕГОРИИ?>
      <div class="main">
            <ul class="tiles-wrap animated" id="wookmark">
              <?php $single = get_single_img();
                  foreach ($single as $key => $info_img): //выводим информацию из таблицы
                    $adress = $info_img["url_500"];
                  ...
                  ?>       
              <li>
                <a href="<?php echo $_SERVER['PHP_SELF']?>?page=gallery&categories=all&photo=<?= +$key+1;?>" title="<?= $info_img["short_description"]; /*адрес картинки*/?>">
                <img class="lazy" data-src="<?php echo $info_img["url_500"];/*адрес картинки с базы*/?>" <?php echo($info[3]); /*адрес картинки с исправлениями*/?>>
                <p><?php echo $info_img["name"];//название?></p>
                </a>
              </li>
              <?php endforeach;?>  
            </ul>
        ...
        <div class="main">
            <ul class="tiles-wrap animated" id="wookmark">
              <?php $single = get_single_img();
                  foreach ($id_cat as $key => $info_img): //выводим информацию из таблицы
                    $adress = $info_img["url_500"];
                    $adress_correction = ltrim($adress , " ../ ");//убирает точки перед URL адрессом
                    $info=getimagesize($adress_correction);//вычитываем информацию из картинки
                  ?>       
              <li>
                <a href="<?php echo $_SERVER['PHP_SELF']?>?page=gallery&categories=<?=$request?>&photo=<?= +$key+1;?>" title="<?= $info_img["short_description"]; /*адрес картинки*/?>">
                <img class="lazy" data-src="<?php echo $info_img["url_500"];/*адрес картинки с базы*/?>" <?php echo($info[3]); /*адрес картинки с исправлениями*/?>>
                <p><?= $info_img["name"];//название?></p>
                </a>
              </li>
              <?php endforeach;?>  
            </ul>
...
<?php
}else{//// если папка пуста - выводем текст?>
    <div class="now_imegen">
    <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="150pt" height = "150pt" viewBox="0 0 1151.000000 1280.000000" preserveAspectRatio="xMidYMid meet">
      ...
      <p><?=help_vars($array_settings,'text_no_img_category');?></p>
    </div>
<?php 
}?>